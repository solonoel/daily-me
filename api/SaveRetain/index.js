      ...
      if (hResult.recordset.length > 0) {
        ...
        if (Link) {
          await pool.request()
            ...
            .query(`
              IF NOT EXISTS (SELECT 1 FROM HeadlineExclusion WHERE UserID=@UserID AND Link=@Link AND UserProfileID=@ProfileID)
                INSERT INTO HeadlineExclusion (UserID, Link, PublishedDate, DeletedDate, UserProfileID)
                VALUES (@UserID, @Link, @PublishedDate, GETDATE(), @ProfileID)
            `);
        }
      }
      await pool.request()
